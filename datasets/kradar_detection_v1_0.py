...
class KRadarDetection_v1_0(Dataset):
    ...
    * We push vis codes without sp cube into util_dataset for readability
    ...
    def show_rpc_cfar_with_lpc(self, dict_item, is_with_dop=False, type_filter=None):
        '''
        * DICT_ITEM['ldr_pc_64']: True
        * DICT_ITEM['rdr_cube']: True
        ...
        *   roi = self.roi_label
        ...
        if is_with_dop:
            ...
            rpc = self.cfar.ca_cfar(dict_item['rdr_cube'], cube_doppler=None)
            # rpc = np.load(dict_item['rdr_sparse_cube'])
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.cfg.DATASET.GET_ITEM['cam_front_img']:
        ...
        * list_dict_batch = list of item (__getitem__)
        ...
        for batch_id, dict_temp in enumerate(list_dict_item):
            for k, v in dict_temp.items():
                if k in ['meta', 'calib', 'cam_front_img']:
